...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        ...
        printf("Use: %s <num-elementos> <u|n>\n", argv[0]);
        puts("     u = uniforme; n = nao-uniforme");
        return EXIT_FAILURE;
    }
    char distribuicao = argv[2][0];
    if (distribuicao != 'u' && distribuicao != 'n') {
        puts("Distribuicao invalida");
        return EXIT_FAILURE;
    }
    int num = atoi(argv[1]);
    int vetor[num];
...
int busca_interpolacao(int* vetor, int inferior, int superior, int chave, int* comparacoes) {
    ...
    int distancia_total_entre_indices = superior - inferior;
    int diferenca_total_entre_chaves = vetor[superior] - vetor[inferior];
    int diferenca_valor_inferior_valor_chave = abs(chave - vetor[inferior]);
    ...
    int distancia_indice_inferior_indice_chave = (diferenca_total_entre_chaves != 0)?
        (float) diferenca_valor_inferior_valor_chave * distancia_total_entre_indices /
