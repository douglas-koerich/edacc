...
// Definicao interna do no de lista simples, nao visivel para usuario da lista
struct Noh {
    Reg elemento;
    struct Noh* proximo;
};

// Definicao pratica da lista encadeada do tipo simples
struct Lista {
    struct Noh* cabeca;
};
...
bool compara(const struct Lista* l1, const struct Lista* l2) {
    if (underflow(l1)) {
        ...
        */
        ...
        if (underflow(l2)) return false;
        else {  // Ambas tem elementos para serem comparados
            struct Noh* p1 = l1->cabeca;
            struct Noh* p2 = l2->cabeca;
            while (p1 != NULL && p2 != NULL) {
                // if (p1->elemento != p2->elemento) {
                // Como os elementos sao estruturas, preciso chamar uma funcao da
                // biblioteca-padrao da linguagem C para comparacao dos bytes de
                // duas areas de memoria de mesmo tamanho (sizeof(Reg))
            ...
            if (p1 == NULL) {   // percorreu toda a lista 1?
                return p2 == NULL;  // true (iguais) se percorreu toda a lista 2 tambem
            }
            else {
                return false;   // soh a lista 2 terminou (e me fez sair do laco)
            }
        }
    }
}
